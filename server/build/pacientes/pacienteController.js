...
export const criarPaciente = async (req, res) => {
    let { cpf, nome, email, senha, estaAtivo, possuiPlanoSaude, endereco, telefone, planosSaude, imagem, historico } = req.body;
    if (!CPFValido(cpf)) {
    ...
    try {
        ...
        const paciente = new Paciente(cpf, nome, email, senhaCriptografada, telefone, planosSaude, estaAtivo, imagem, historico);
...
export const lerPacientes = async (req, res) => {
...
export const lerPaciente = async (req, res) => {
    const { id } = req.params;
...
export const listaConsultasPaciente = async (req, res) => {
    const { id } = req.params;
    const paciente = await AppDataSource.manager.findOne(Paciente, {
        where: { id }
    });
...
export const atualizarPaciente = async (req, res) => {
    let { nome, email, senha, estaAtivo, telefone, possuiPlanoSaude, planosSaude, cpf, imagem, historico } = req.body;
    const { id } = req.params;
    if (!CPFValido(cpf)) {
...
export const atualizarEnderecoPaciente = async (req, res) => {
    const { id } = req.params;
...
export const desativaPaciente = async (req, res) => {
    const { id } = req.params;
    const paciente = await AppDataSource.manager.findOne(Paciente, {
        where: { id }
    });
