...
import { estaAtivoEspecialista, estaAtivoPaciente, validaAntecedenciaMinima, validaClinicaEstaAberta, pacienteEstaDisponivel, especialistaEstaDisponivel } from './consultaValidacoes.js';
import { mapeiaLembretes } from '../utils/consultaUtils.js';
import { AppError, Status } from '../error/ErrorHandler.js';
export const criaConsulta = async (req, res) => {
    ...
    if (!validaAntecedenciaMinima(data, 30)) {
        throw new AppError('A consulta deve ser agendada com 30 minutos de antecedência', Status.BAD_REQUEST);
    }
...
export const listaConsultas = async (req, res) => {
...
export const buscaConsultaPorId = async (req, res) => {
    const { id } = req.params;
...
export const deletaConsulta = async (req, res) => {
    const { id } = req.params;
    ...
    if (!validaAntecedenciaMinima(consulta.data, HORA)) {
        throw new AppError('A consulta deve ser desmarcada com 1 dia de antecedência');
    }
